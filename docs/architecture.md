...
**再生位置に追従するスクロール表示(2026-09-19追加)**: 波形要素は「曲全体を固定表示」の他に、
指定秒数(`windowSeconds`、3〜30秒)の範囲を再生位置中心に切り出してスクロール表示できる
(表示する秒数が実質的な「流れる速さ」の調整値になる)。この用途では
`/waveform-render/deck/{n}`に`scale=1`(ネイティブ解像度)を指定して1曲につき1回だけ全体を
取得し、どのフレーム範囲を切り出すかはクライアント側で毎フレーム計算する(サーバー側の
ポーリング間隔自体は変えていない)。トラック境界付近では表示窓が切り詰められ、
プレイヘッドが中央からずれる。グリッド表示(`showGrid`)を有効にすると、中央の水平線と
見やすい間隔に自動調整された縦の時間目盛りが波形の下に描画される。

**レイヤー管理・回転・背景色(2026-09-19追加)**: `/editor`の各要素パネルの持ち手をドラッグ&ドロップ
することで、`layout.elements`配列内の順序(=DOM描画順、そのままz順になる)を入れ替えられる
ようにした。また全要素タイプ共通で`rotation`(度数)を持たせ、CSSカスタムプロパティ`--rot`経由で
切り替えエフェクトのtransformと共存させている(切り替えアニメーションのtranslate/scaleと
回転を同時に適用するため)。背景色は`layout.backgroundColor`("transparent"またはHEX)で、
OBS向けの透過を既定としつつブラウザ単体での確認等に任意の背景色を設定できる。

**フォントサイズの基準統一(2026-09-19追加)**: `/editor`のプレビューcanvas(960x540固定)と
実際のOverlay(OBSの実解像度、多くは1920x1080)とで同じ`fontSize`の生px値をそのまま適用すると
見た目の大きさがズレるため、`fontSize`は「1920x1080のステージを基準にした値」として扱い、
実際のステージ高さに応じて比例縮尺する(`scaledFontSizePx`)。

**波形のプレイヘッド位置(2026-09-19追加)**: スクロール表示の波形で、再生位置を画面のどこに
固定するか(`playheadPosition`、0=左端・50=中央[既定]・100=右端)を`/editor`で設定できる。
描画は「フレーム番号iを`anchorPx + (i - centerFrame) * barWidth`でx座標に変換する」という
単純な線形写像(`frameToX`)に統一し、中央固定だった頃の非対称なクランプ処理をやめた
(曲の先頭/終端では単に表示できるフレームが尽きて空白になるだけで、スケールが崩れない)。
グリッドの縦目盛りも同じ写像を使うため、バーの表示位置とズレることがない。
なお、実機のCDJを遠隔シークすることはPro DJ Linkの仕様上できないため、`/`に`?debug`を付けて
アクセスした時だけ現在の曲の実波形を任意の位置(割合指定)でテスト表示できるパネルも用意した
(通常のOBS用URLでは生成されず配信画面に影響しない)。

**波形取得時のdurationMs不整合(2026-09-19修正)**: 曲切り替え直後、再生位置情報(`vars`の
`duration-ms`)は毎tick最新の値に更新される一方、波形データの取得(1曲につき1回、`scale=1`)は
少し遅れるため、新しい曲のdurationMsと古い曲の波形データ(frameCount)を組み合わせてスクロール窓を
計算してしまい表示が崩れる瞬間があった。取得したsegmentsにはその時点のdurationMsを固定して
持たせ(`waveformData[id].durationMs`)、描画時は常にそのペアで計算するよう修正した。

**テキストのはみ出し処理(2026-09-19追加)**: テキスト要素に`overflowMode`
(`ellipsis`[既定・省略] / `wrap`[折り返し] / `scroll`[横スクロール])を追加した。
`scroll`は実際に枠からはみ出す場合だけCSSアニメーション(`marquee-scroll`)を有効にする
(収まっているのに動き続けるのを避けるため、`scrollWidth`と`clientWidth`を比較して判定)。
テキストは常に`.text-inner`という子`<span>`に入れる構造にしている(`scroll`のtransform
アニメーション対象にするため。`ellipsis`はこの子`<span>`がinline要素のままなら親の
`text-overflow: ellipsis`が正しく効くことを利用している)。
